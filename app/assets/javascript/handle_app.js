...
function App() {
    ...
    // Removing selectize from datatables
    this.removeSelectizeFromTable = function (element_name) {
        if($('select[name = "' + element_name + '"]').hasClass("selectized")){
            $('select[name = "' + element_name + '"]').selectize()[0].selectize.destroy();
            $('select[name = "' + element_name + '"]').val(25);
        }
        $('select[name = "' + element_name + '"]').addClass('datatable-row-options');
    };
        ...
        if (!config || typeof config == 'undefined' ) {
            config = {
                ...
                drawCallback: function (settings) {
                    try {
                        caller.datatableInitDraw(settings);
                    } catch (e) {
